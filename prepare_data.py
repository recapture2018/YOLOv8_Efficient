...
def convert_annotation(dir_path, dataset_name, image_id):
    ...
    :return:
    '''
    with open(dir_path + dataset_name + f'/VOC2007/Annotations/{image_id}.xml') as in_file:
        out_file = open(
            dir_path + dataset_name + f'/VOC2007/YOLOLabels/{image_id}.txt',
            'w',
        )
...
def trans_prepare_config(dir_path='data/', dataset_name='VOCdevkit_xxx'):
    ...
    with open(f"{dir_path}yolov5_train.txt", 'w') as train_file:
        test_file = open(f"{dir_path}yolov5_val.txt", 'w')
    test_file.close()
    with open(os.path.join(f"{dir_path}yolov5_train.txt"), 'a') as train_file:
        test_file = open(os.path.join(f"{dir_path}yolov5_val.txt"), 'a')
        ...
        for i in tqdm(range(0, len(list_imgs))):
            ...
            if os.path.isfile(path):
                ...
                annotation_name = f'{nameWithoutExtention}.xml'
                annotation_path = os.path.join(annotation_dir, annotation_name)
                label_name = f'{nameWithoutExtention}.txt'
